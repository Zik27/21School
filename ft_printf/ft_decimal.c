...
/*   Updated: 2019/01/31 15:05:00 by djast            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_print_width(t_qual *qual, int size_nbr)
{
	...
	if (qual->plus == 1 && qual->sign == 0 && qual->precision == -1 && qual->zero == 1 && qual->minus == 0 && qual->unsint == 0)
		write(1, "+", 1);
	if (qual->space == 1 && qual->sign == 0 && qual->plus == 0 && qual->unsint == 0)
		write(1, " ", 1);
	if (qual->sign == 1 && qual->precision == -1 && (qual->width == -1 || qual->zero == 1) && qual->minus == 0 && qual->unsint == 0)
		write(1, "-", 1);
	if (qual->precision == -1 && qual->zero == 1 && qual->minus == 0)
		print_char = '0';
	else
		print_char = ' ';
	real_precision = qual->precision < size_nbr ? size_nbr : qual->precision;
	real_width = qual->plus == 1 ? qual->width - real_precision - 1 + qual->unsint : 
				 qual->width - real_precision - qual->sign - qual->space;
	while (real_width-- > 0)
		write(1, &print_char, 1);
	if (qual->sign == 1 && qual->precision == -1 && qual->width != -1 && qual->zero == 0 && qual->unsint == 0 && qual->minus == 0)
		write(1, "-", 1);
	if (qual->plus == 1 && qual->sign == 0 && qual->precision == -1 && qual->zero == 0 && qual->unsint == 0 && qual->minus == 0)
		write(1, "+", 1);
}
...
{
	int real_precision;
	if (qual->plus == 1 && qual->sign == 0 && qual->unsint == 0 && (qual->precision != -1 || qual->minus == 1))
		write(1, "+", 1);
	if (qual->sign == 1 && (qual->precision != -1 || qual->minus == 1) && qual->unsint == 0)
		write(1, "-", 1);
...
int ft_d(intmax_t nbr, t_qual *qual)
{
	...
	if (nbr == 0 && qual->precision == 0)
		size_nbr--;
	return (qual->space == 1 || qual->plus == 1) && qual->sign == 0 ? ft_max(3, qual->width, qual->precision, size_nbr + qual->sign + 1): 
