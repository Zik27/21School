...
** minus - {Флаги} "-" - выводимое значение выравнивается по
** левому краю в пределах минимальной ширины поля.
** plus - {Флаги} "+" - всегда указывать знак для
** выводимого десятичного числового значения.
** space - {Флаги} " " - помещать перед результатом пробел,
** если первый символ значения не знак.
** hash - {Флаги} "#" - «альтернативная форма» вывода значения
** (Для 2, 8, 16 СС).
** zero - {Флаги} "0" - дополнять поле до ширины,
** указанной в поле ширина управляющей последовательности, символом 0.
**-----------------------------------------------------------------------------
** width - {Ширина} - указывает min ширину поля (включая знак для чисел).
**-----------------------------------------------------------------------------
** precision - {Точность} - минимальное количество символов,
** которое должно появиться при обработке типов d, i, o, u, x, X;
** precision - {Точность} - минимальное количество символов,
** которое должно появиться после точки при обработке типов a, A, e, E, f, F;
** precision - {Точность} - max количество значащих символов для типов g и G;
** precision - {Точность} - max число символов,
** которые будут выведены для типа s;
**-----------------------------------------------------------------------------
